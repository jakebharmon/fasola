...
get_minutes <- function(
  ...
  # Download database details and connect R to it
  ...
  # Run the "I'm Feeling Lucky" function which prints the lyrics of a random song
  feeling_lucky()

  # Begin joining/parsing data
  tryCatch({
    cli::cli_progress_step("Joining tables from database")
  ...
  # Base columns
  base_cols <- c("leader_id", "leader_name" = "name.x", "song_id",
                 "song_title" = "title", "minutes_id", "year" = "Year",
                 "date_start", "date_end")

  # Extra columns as defined by user input
  ...
  result <- dplyr::select(result, dplyr::any_of(keep)) |>
    ...
    dplyr::mutate(dplyr::across(where(is.character), \(x) iconv(x, from = "UTF-8", to = "ASCII//TRANSLIT")))
